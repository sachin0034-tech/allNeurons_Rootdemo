...
Dark green / gold theme matching the reference slides.
...
Slides:
  ...
  3. Store Operations     — traffic, CVR, AOV, top/bottom stores + narrative
...
# ── Palette ───────────────────────────────────────────────────
BG    = RGBColor(0x1C, 0x2B, 0x1E)
CARD  = RGBColor(0x24, 0x33, 0x27)
GOLD  = RGBColor(0x8B, 0x7D, 0x3A)
CREAM = RGBColor(0xF5, 0xF0, 0xE8)
MUTED = RGBColor(0x9A, 0xAB, 0x9C)
POS   = RGBColor(0x4C, 0xAF, 0x7C)
NEG   = RGBColor(0xC0, 0x39, 0x2B)
AMB   = RGBColor(0xD4, 0xA0, 0x17)
BDR   = RGBColor(0x3A, 0x4F, 0x3D)
WHITE = RGBColor(0xFF, 0xFF, 0xFF)
DARK  = RGBColor(0x0F, 0x1A, 0x11)
...
def _header(slide, title, subtitle=""):
    _rect(slide, 0, 0, 13.33, 0.82, CARD)
    _rect(slide, 0, 0.80, 13.33, 0.04, GOLD)
    _txt(slide, title, 0.20, 0.05, 12.9, 0.44, 11, GOLD, bold=True)
    if subtitle:
        _txt(slide, subtitle, 0.20, 0.47, 12.9, 0.28, 8, MUTED, italic=True)


def _section_label(slide, text, x, y, w):
    _rect(slide, x, y + 0.16, w, 0.018, GOLD)
...
def _kpi(slide, x, y, w, label, val_str, ly_str, chg, bps=False):
    H = 0.90
    _rect(slide, x, y, w, H, CARD, BDR, 0.5)
    _txt(slide, label, x+0.08, y+0.04, w-0.16, 0.18, 7, GOLD, bold=True)
    _txt(slide, val_str, x+0.08, y+0.21, w-0.16, 0.36, 16, CREAM, bold=True)
...
def _td(slide, text, x, y, w, h, rgb=CREAM, bg=CARD, bold=False,
        align=PP_ALIGN.CENTER):
    _rect(slide, x, y, w, h, bg, BDR, 0.25)
...
def _bullets(slide, items, x, y, w, h, color=CREAM, pt=8.5, prefix="•"):
    box = slide.shapes.add_textbox(Inches(x), Inches(y), Inches(w), Inches(h))
    tf = box.text_frame
    tf.word_wrap = True
    first = True
...
def _source_line(slide, sources, x, y, w):
    ...
    for t in texts:
        ...
        run.font.color.rgb = RGBColor(0x5A, 0x7A, 0x5E)
...
def _slide_exec_summary(prs, metrics: dict, narr: dict):
    ...
    if bullets:
        half = len(bullets) // 2
        _bullets(slide, bullets[:half], 0.20, DY, 6.40, 1.80, CREAM, 9)
        _bullets(slide, bullets[half:], 6.80, DY, 6.33, 1.80, CREAM, 9)
...
def _slide_revenue(prs, metrics: dict, fp_df, narr: dict):
    ...
    if not fp_df.empty:
        for i, (_, row) in enumerate(fp_df.iterrows()):
            ...
            vals = [
                ...
                (_fmt_M(cy_s),   CREAM, False, PP_ALIGN.CENTER),
            ...
            bg_row = CARD if i % 2 == 0 else RGBColor(0x1F, 0x2F, 0x22)
            rx2 = TX
    ...
    _bullets(slide, bullets, NX, NY, NW, 3.60, CREAM, 8.5)
    ...
    if not fp_df.empty:
        ...
        for _, row in fp_df.iterrows():
            ...
            _txt(slide, _fmt_pct(rfp), NX+0.65, sy, 0.70, 0.28, 7.5, CREAM)
            chg_c = _chg_color(rfp - rly)
            _rect(slide, NX+1.40, sy+0.04, max(0.05, (rfp / 0.85) * (NW-1.55)),
                  0.20, chg_c)
            sy += 0.30
            if sy > 7.25:
                break
...
def _slide_operations(prs, metrics: dict, st_df, narr: dict):
    ...
    if not st_df.empty:
        for i, (_, row) in enumerate(st_df.iterrows()):
            ...
            ly_tr = _f(row.get("LY_TRAFFIC", tr or 1))
            cv    = _f(row.get("CVR"))
            ...
            vals = [
                ...
                (_fmt_M(ns),        CREAM, False, PP_ALIGN.CENTER),
                (_fmt_num(tr),      CREAM, False, PP_ALIGN.CENTER),
                (_fmt_pct(cv),      CREAM, False, PP_ALIGN.CENTER),
                (f"${a:.2f}",       CREAM, False, PP_ALIGN.CENTER),
                (f"{u:.2f}",        CREAM, False, PP_ALIGN.CENTER),
    ...
    _bullets(slide, bullets[:2], 0.20, ny, 6.40, 0.80, CREAM, 8)
    _bullets(slide, bullets[2:], 6.80, ny, 6.33, 0.80, CREAM, 8)
...
def _slide_supply_chain(prs, metrics: dict, narr: dict):
    ...
    for i, (lbl, val, ly_v, chg, bps) in enumerate(kpis):
        ...
        if chg is None:
            ...
            _rect(slide, kx, Y0, KW, 0.90, CARD, BDR, 0.5)
            _txt(slide, lbl, kx+0.08, Y0+0.04, KW-0.16, 0.18, 7, GOLD, bold=True)
            _txt(slide, val, kx+0.08, Y0+0.21, KW-0.16, 0.36, 16, CREAM, bold=True)
    ...
    rows = [
        ("On-Time Delivery %",      _fmt_pct(otd),      _fmt_pct(ly_otd),
         _fmt_bps(otd-ly_otd),     POS if otd >= ly_otd else NEG, "On Track" if otd >= 0.92 else "Watch"),
        ("Freight Cost Index",      _fmt_yoy(freight),  "Base",
         _fmt_yoy(freight),        POS if freight < 0 else NEG, "Favourable" if freight < 0 else "Over"),
        ("Avg. Inventory Age (days)",f"{inv_aging:.0f}", "—",
         "—",                      AMB, "Monitor"),
        ("Fill Rate %",             _fmt_pct(fill_rate), _fmt_pct(ly_fill),
         _fmt_bps(fill_rate-ly_fill), POS if fill_rate >= ly_fill else NEG,
         "On Track" if fill_rate >= 0.95 else "Risk"),
        ("Inventory Turns (ann.)",  f"{inv_turns:.1f}x", f"{ly_turns:.1f}x",
         f"{inv_turns-ly_turns:+.1f}x", POS if inv_turns >= ly_turns else NEG,
    ...
    for i, (metric, cur, ly_v, var, chg_c, status) in enumerate(rows):
        bg_row = CARD if i % 2 == 0 else RGBColor(0x1F, 0x2F, 0x22)
        vals = [
            (metric,  CREAM, True, PP_ALIGN.LEFT),
            (cur,     CREAM, False, PP_ALIGN.CENTER),
    ...
    _bullets(slide, bullets[:2], 0.20, ny, 6.40, 0.75, CREAM, 8)
    _bullets(slide, bullets[2:], 6.80, ny, 6.33, 0.75, CREAM, 8)
...
def _slide_risks(prs, narr: dict):
    ...
    # ── LEFT: Key Risks ───────────────────────────────────────
    LX, LY, LW = 0.20, 0.92, 6.30
    _rect(slide, LX, LY, LW, 5.70, CARD, BDR, 0.5)
    ...
    for risk in risks:
        # red bullet dot
        _rect(slide, LX+0.18, ry+0.06, 0.10, 0.10, NEG)
        _txt(slide, risk, LX+0.36, ry, LW-0.50, 0.60, 8.5, CREAM, wrap=True)
        ry += 0.70

    # ── RIGHT: Opportunities ──────────────────────────────────
    RX = 6.83
    RW = 13.33 - RX - 0.20
    _rect(slide, RX, LY, RW, 5.70, CARD, BDR, 0.5)
    ...
    oy = LY + 0.55
    for opp in opps:
        _rect(slide, RX+0.18, oy+0.06, 0.10, 0.10, POS)
        _txt(slide, opp, RX+0.36, oy, RW-0.50, 0.60, 8.5, CREAM, wrap=True)
        oy += 0.70
...
def build_deck(kpi: dict, fp_df, st_df, reg_df,
               narratives: dict, week_label: str = "") -> bytes:
    """
    Build a 5-slide executive PPTX and return the bytes.
    ...
    if not st_df.empty and "NET_SALES" in st_df.columns and "LY_NET_SALES" in st_df.columns:
        st_df2 = st_df.copy()
        st_df2["_yoy"] = (st_df2["NET_SALES"].astype(float) - st_df2["LY_NET_SALES"].astype(float)) / st_df2["LY_NET_SALES"].astype(float).replace(0, float("nan"))
